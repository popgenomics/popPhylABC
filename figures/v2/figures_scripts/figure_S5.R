...
# plot HH
plot(log10(x$divAB_avg), pmig_HH, ylab ="" , xlab = "gross synonymous divergence", col = "white", cex.lab = 1.25, xaxt = "n", yaxt = "n", xlim=c(log10(1e-3), log10(1)), cex.axis=1.25)
par(las=3)
mtext(side=2, text=expression(P["ongoing migration"]), line=2.25, cex=1.5)
par(las=1)
axis(side = 1, at = c(0, -1, -2, -3), labels = c(1, 0.1, 0.01, expression(paste("10"^"-3"))), cex.axis=1.25)
axis(side = 2, at = c(0, 0.2, 0.4, 0.6, 0.8, 1), cex.axis=1.25)
mtext(side = 3, text = "hetero M + Ne", cex = 1.5)

div_NA = log10(sort(x[is.na(bilanHH),]$divAB_avg))
div_Mig = log10(sort(x[which(bilanHH=="Migration"),]$divAB_avg))
div_NoMig = log10(sort(x[which(bilanHH=="NoMigration"),]$divAB_avg))
xA1 = div_NoMig[1]
...
for(i in 1:length(bilanHH)){
	...
	if(testA != testB & testB!="NA"){
		Arrows(log10(x$divAB_avg)[i], pmig_HH[i], log10(x$divAB_avg)[i], pmig_Hh[i], arr.length=0.1, arr.type="triangle", arr.width=0.1, lty=4, col=rgb(0,1,0, 0.75), lwd = 2.75)
	}
	if(testA != testB & testB=="NA"){
		Arrows(log10(x$divAB_avg)[i], pmig_HH[i], log10(x$divAB_avg)[i], pmig_Hh[i], arr.length=0.1, arr.type="triangle", arr.width=0.1, lty=4, col=rgb(1,0,0, 0.75), lwd = 2.75)
	}
}
